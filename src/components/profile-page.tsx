...
export function ProfilePage({ dict, lang }: { dict: any; lang: string }) {
      ...
      <div className="container py-8 mx-auto p-4">
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">{dict.profile.notFound}</h2>
            <p className="text-muted-foreground mb-4">{dict.profile.notFoundDesc}</p>
            <Button asChild>
              <Link href="/snippets">{dict.profile.browseAllSnippets}</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadPromise = (async () => {
      ...
      try {
        ...
        setUser(data);
        setSelf(data);
        return dict.profile.updateAvatarSuccessfully;
      } catch (err) {
    ...
    toast.promise(uploadPromise, {
      loading: `${dict.profile.uploading}`,
      success: (msg) => msg,
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {userId === self?.id && (
                      <Dialog open={open} onOpenChange={setOpen}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>{dict.profile.editName}</DialogTitle>
                          </DialogHeader>
                          ...
                          <DialogFooter>
                            ...
                            <Button onClick={handleSavingUsername} disabled={saving}>
                              {saving && <Spinner />} {dict.profile.save}
                            </Button>
                          </DialogFooter>
                        </DialogContent>
                      </Dialog>
        ...
        <div>
          {user && (
            <h2 className="text-2xl font-bold mb-6">
              {dict.profile.snippetsBy} {user.username}
            </h2>
          )}
          {fetchingSnippets ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                <p className="text-muted-foreground">{dict.profile.noSnippets}</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
